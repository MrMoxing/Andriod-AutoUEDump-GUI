...
unsigned long long zip_entry_data_offset(struct zip_t* zip)
{
    if (!zip)
        return 0;

    mz_uint8 header[MZ_ZIP_LOCAL_DIR_HEADER_SIZE];
    memset(header, 0, MZ_ZIP_LOCAL_DIR_HEADER_SIZE);

    if (MZ_ZIP_LOCAL_DIR_HEADER_SIZE != zip->archive.m_pRead(zip->archive.m_pIO_opaque, zip->entry.header_offset, header, MZ_ZIP_LOCAL_DIR_HEADER_SIZE))
        return 0;

    unsigned long long off = zip->entry.header_offset;
    off += MZ_ZIP_LOCAL_DIR_HEADER_SIZE;
    off += MZ_READ_LE16(header + MZ_ZIP_LDH_FILENAME_LEN_OFS);
    off += MZ_READ_LE16(header + MZ_ZIP_LDH_EXTRA_LEN_OFS);
